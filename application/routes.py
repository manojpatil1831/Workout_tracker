...
#         workout = Workout(date=workout_date, user_id=user.id)
#         exercise_count = int(request.form['exercise_count'])
...
def add_workout():
    if request.method == 'POST':
        ...
        # Get the date from the form
        workout_date1 = request.form.get('date')
        workout_time=request.form.get('time')
        
        # Convert the date string to a datetime object
        workout_date = datetime.strptime(f"{workout_date1} {workout_time}", '%Y-%m-%d %H:%M')
